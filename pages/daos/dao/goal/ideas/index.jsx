...
export default function GrantIdeas() {
	...
	async function fetchContractData() {
		...
		try {
			if (contract && id) {
				...
				const totalComments = await contract.getMsgIDs(Number(id)).call() //Getting total comments (Number) of this idea
				// const arr = []
				for (let i = 0; i < Object.keys(totalComments).length; i++) {
					//total comments number Iteration
					const commentId = Number(totalComments[i]);
					let commentInfo = await contract.all_messages(commentId).call();
					...
					const totalReplies = await contract.getReplyIDs(Number(commentId)).call() //Getting total replies (Number) of this comment
					for (let i = 0; i < Object.keys(totalReplies).length; i++) {
						const replyId = Number(totalReplies[i]);
						let replyInfo = await contract.all_replies(replyId).call();
	...
	async function PostComment(e) {
		...
		let newComment = {
			address: window?.tronWeb?.defaultAddress?.base58.toString(),
			message: Comment,
	...
	async function saveMessage(newComment) {
		await sendTransaction(contract.sendMsg(Number(ideaId), JSON.stringify(newComment)));
		removeElementFromArrayBYID(emptydata, 0, setemptydata)
		console.log("Saved Messages")
	}
	async function sendReply(replyText, MessageId, MessageIndex) {
		let replyLatestId = Number(await contract._reply_ids().call());
		let newReply = {
			id: replyLatestId,
			message: replyText,
			address: window?.tronWeb?.defaultAddress?.base58.toString(),
		...
		await sendTransaction(contract.sendReply(Number(MessageId), JSON.stringify(newReply)));
		removeElementFromArrayBYID(emptydata, 0, setemptydata)
			...
			<div className={`${styles.container} flex flex-col items-center gap-8`}>
				<div className={`${styles.title} flex flex-col gap-2`}>
					...
					<div>
						<Loader
							element={
								<a className="font-medium text-[#0000ff]" href={`https://testnet.mintscan.io/neutron-testnet/account/${IdeasURI.wallet}`} style={{color:"var(--title-a-text)"}}  rel="noreferrer" target="_blank">
									{IdeasURI.wallet}
								</a>
							}
							width={"80%"}
