...
export function Nav(): JSX.Element {
  ...
  async function fetchInfo() {
    if (typeof window.keplr === "undefined") {
      ...
      window.document.getElementById("installKeplr").style.display = "";
      running = false;
      return;
    }else{
      window.document.getElementById("withoutSign").style.display = "";
      window.document.getElementById("withSign").style.display = "none";
      window.document.getElementById("installKeplr").style.display = "none";
    }
    if (window.localStorage.getItem("login-type") === "keplr") {
      if (window.keplr.tronWeb !== false) {
        try {
        ...
        <li className="Nav walletstatus flex flex-1 justify-end">
          ...
          >
            <div className="wallet">
              <Button variant="tertiary" onClick={() => { window.open("https://chrome.google.com/webstore/detail/keplr/dmkamcknogkgcdfhhbddcghachkejeap", "_blank") }}> Keplr</Button>
            </div>
          </div>
