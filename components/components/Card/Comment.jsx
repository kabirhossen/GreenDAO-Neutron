...
const Comment = ({address, date, message, replies = [], sendReply, MessageIndex, MessageID}) => {
	...
	return (
		<div className="topic-post clearfix regular">
			<article className="boxed onscreen-post" style={{display: "block", width: "100%"}}>
				<div className="row" style={{display: "flex", flexDirection: "row"}}>
					<div className={style.topicAvatar}>
						<div className="post-avatar">
							<a className="trigger-user-card main-avatar " href={`https://testnet.mintscan.io/neutron-testnet/account/${address}`} rel="noreferrer" target="_blank" aria-hidden="true" tabIndex={-1}>
						...
						<div role="heading" className={style.TopicMetaData}>
							<div className={style.TriggerUserCard}>
								<span className="font-bold text-piccolo">
									<a href={`https://testnet.mintscan.io/neutron-testnet/account/${address}`} style={{color:"var(--title-a-text)"}} rel="noreferrer" target="_blank">
										{address}
									</a>
								</span>
							</div>
							<div className={style.PostInfos}>
								<div className="post-info post-date" style={{flex: "0 0 auto", marginRight: 0}}>
									<a className="widget-link post-date" title="Post date">
										<span style={{color: "#8e8e8e", whiteSpace: "nowrap"}}>{formatDistance(new Date(date), new Date(), {addSuffix: true})}</span>
									</a>
								</div>
							</div>
						</div>
...
c0.009-0.885,0.017-1.768,0.017-2.65C492.425,258.31,374.944,142.091,228.398,137.833z"
				...
				{replies.map((item, index) => (
					<div key={item.id} className="row" style={{display: "flex", justifyContent: "center", padding: "1rem", paddingLeft: "4rem", paddingRight: "0"}}>
						<div className={style.topicAvatar}>
							<div className="post-avatar">
								<a className="trigger-user-card main-avatar " href={`https://testnet.mintscan.io/neutron-testnet/account/${item.address}`} rel="noreferrer" target="_blank" aria-hidden="true" tabIndex={-1}>
									<svg width={45} height={45} xmlns="http://www.w3.org/2000/svg"  style={{fill:"var(--foreground)"}} viewBox="0 0 459 459">
										<g>
											<g>
												<path d="M229.5,0C102.53,0,0,102.845,0,229.5C0,356.301,102.719,459,229.5,459C356.851,459,459,355.815,459,229.5    C459,102.547,356.079,0,229.5,0z M347.601,364.67C314.887,393.338,273.4,409,229.5,409c-43.892,0-85.372-15.657-118.083-44.314    c-4.425-3.876-6.425-9.834-5.245-15.597c11.3-55.195,46.457-98.725,91.209-113.047C174.028,222.218,158,193.817,158,161    c0-46.392,32.012-84,71.5-84c39.488,0,71.5,37.608,71.5,84c0,32.812-16.023,61.209-39.369,75.035    c44.751,14.319,79.909,57.848,91.213,113.038C354.023,354.828,352.019,360.798,347.601,364.67z" />
						...
						<div className={style.clearfix}>
							<div role="heading" className={style.TopicMetaData}>
								<div className={style.TriggerUserCard}>
									<span className="font-bold text-piccolo">
										<a href={`https://testnet.mintscan.io/neutron-testnet/account/${item.address}`} style={{color:"var(--title-a-text)"}} rel="noreferrer" target="_blank">
											{item.address}
										</a>
									</span>
								</div>
								<div className={style.PostInfos}>
									<div className="post-info post-date" style={{flex: "0 0 auto", marginRight: 0}}>
										<a className="widget-link post-date" title="Post date">
											<span style={{color: "#8e8e8e", whiteSpace: "nowrap"}}>{formatDistance(new Date(item.date), new Date(), {addSuffix: true})}</span>
										</a>
									</div>
								</div>
							</div>
