...
export default function DonateCoin({ideasid,show, onHide, address}) {
	...
	async function DonateCoinSubmission(e) {
		...
		setTransaction({
		   ...
		   await sendTransaction(contract.add_donation(Number(ideasid),(Number(amount.value) * 1e6).toFixed(0)));		
	...
	async function LoadData() {
		...
		setBalance((Balance / 1e6).toFixed(5) + " NTRN");
		setCurrentAddress(window?.tronWeb?.defaultAddress?.base58.toString());
	}
	useEffect(() => {
		LoadData();
	}, [show]);
